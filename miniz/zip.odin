...
file_read_func :: #type proc "c" (pOpaque: rawptr, file_ofs: mz_uint64, pBuf: rawptr, n: size_t) -> size_t
file_write_func :: #type proc "c" (pOpaque: rawptr, file_ofs: mz_uint64, pBuf: rawptr, n: size_t) -> size_t
file_needs_keepalive :: #type proc "c" (pOpaque: rawptr) -> mz_bool
...
foreign lib {
	zip_reader_init_mem :: proc(pZip: ^zip_archive, pMem: rawptr, size: size_t, flags: mz_uint) -> mz_bool ---
	zip_get_last_error :: proc(pZip: ^zip_archive) -> zip_error ---
	zip_get_error_string :: proc(mz_err: zip_error) -> cstring ---
	zip_reader_get_num_files :: proc(pZip: ^zip_archive) -> mz_uint ---
	zip_reader_is_file_a_directory :: proc(pZip: ^zip_archive, file_index: mz_uint) -> mz_bool ---
	zip_reader_is_file_supported :: proc(pZip: ^zip_archive, file_index: mz_uint) -> mz_bool ---
	zip_reader_get_filename :: proc(pZip: ^zip_archive, file_index: mz_uint, pFilename: ^byte, filename_buf_size: mz_uint) -> mz_uint ---
	zip_reader_extract_to_callback :: proc(pZip: ^zip_archive, file_index: mz_uint, pCallback: file_write_func, pOpaque: rawptr, flags: mz_uint) -> mz_bool ---
	zip_end :: proc(pZip: ^zip_archive) -> mz_bool ---
}

@private
zip_set_error :: #force_inline proc "contextless" (zip: ^zip_archive, err_num: zip_error) -> mz_bool {
	if zip != nil {
...
@private
zip_file_write_callback :: proc "c" (opaque: rawptr, file_ofs: mz_uint64, buf: rawptr, n: size_t) -> size_t {
	context = runtime.default_context() // TODO: remove this when os procedures are made contextess
	written, _ := os.write((cast(^os.Handle)opaque)^, slice.bytes_from_ptr(buf, auto_cast n))
	return auto_cast written
}

zip_reader_extract_to_file :: proc(zip: ^zip_archive, file_index: mz_uint, dst_filename: string, flags: mz_uint) -> mz_bool {
